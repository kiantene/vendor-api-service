...
public class BalanceController extends AbstractBalanceController<BalanceRequest, CommonResponse> {
    ...
    protected BalanceController(BalanceContextMapper<BalanceRequest> requestMapper, BalanceVendorResponseMapper<CommonResponse> responseMapper, WalletBalanceService walletService) {
        super(requestMapper, responseMapper, walletService);
    }

    @PostMapping(path = Endpoints.BALANCE)
    @VendorExceptionHandler(className = Endpoints.CLASS_NAME)
    public ResponseEntity<CommonResponse> balance(@Valid @RequestBody BalanceRequest request) {
