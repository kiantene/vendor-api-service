...
public class BalanceService {
    ...
    public CommonVo balance(HttpRequestLog httpRequestLog, String traceId) {
        ...
        try {
            ...
            BalanceDto balanceDto = VendorService.convertQueryStringToDtoUrlDecode(body, BalanceDto.class);
            String formatedMessageDto = balanceDto.getMessage();
            MessageDto messageDto = HttpService.convertJsonToDto(formatedMessageDto, MessageDto.class);


            // 2. Validate request parameters (Non-database calls)
            this.doValidation(messageDto);
            ...
            this.doVerification(balanceDto, messageDto, gameSession);
...
//            httpService.logError(httpRequestLog, e);
    ...
    private void doValidation(MessageDto messageDto) throws InvalidRequestException {
        // General validation
        ValidationUtils.validateRequest(messageDto);
    }

    private void doVerification(BalanceDto balanceDto, MessageDto messageDto, GameSession gameSession) throws AuthenticationException,
