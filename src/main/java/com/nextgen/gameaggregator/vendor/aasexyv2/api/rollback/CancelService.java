...
import com.nextgen.gameaggregator.vendor.aasexyv2.constant.ResponseCodes;
import com.nextgen.gameaggregator.vendor.aasexyv2.dto.RequestDto;
import com.nextgen.gameaggregator.vendor.aasexyv2.service.VendorService;
import com.nextgen.gameaggregator.vendor.aasexyv2.vo.BalanceVo;
import com.nextgen.gameaggregator.vendor.aasexyv2.vo.GeneralVo;
...
public class CancelService {
    ...
    public CancelVo cancel(HttpRequestLog httpRequestLog, HttpServletRequest request, String traceId) {
        ...
        try {
            ...
            CancelTransactionsDto txn = dto.getMessage().getTxns().get(0);
            try {
                gameSession = gameSessionService.getGameSessionByVendorPlayerUsername(txn.getUserId());
            } catch (AuthenticationException e) {
                gameSession = gameSessionService.generateNewSessionToken(txn.getUserId());
                String gameCodeWithTable = txn.getGameCode() + "_" + txn.getGameInfo().getTableId();
                gameSessionService.updateByVendorGameCode(gameSession, gameCodeWithTable);
    ...
    private void doVerification(CancelTransactionsDto cancelTransactionsDto, GameSession gameSession) throws
        ...
        ValidationUtils.isEquals(gameSession.getVendorGameCode(), cancelTransactionsDto.getGameCode()+"_"+cancelTransactionsDto.getGameInfo().getTableId(), GameNotSupportedException::new);
    ...
    private BalanceVo processData(CancelTransactionsDto cancelTransactionsDto, HttpServletRequest request, GameSession gameSession) {
        ...
        try {
            ...
            gameSession = vendorService.verifyAndRegenerateNewVendorGameCodeForGameSession(cancelTransactionsDto.getGameCode()+"_"+cancelTransactionsDto.getGameInfo().getTableId(), gameSession);
