...
import com.nextgen.gameaggregator.vendor.gpkiconic.constant.Platforms;
...
@Service
public class GameUrlService extends BaseGameUrlService<IconicGameUrlVo> {
    String apiToken = "api_token";
    String platform = "platform";
    String timestamp = "timestamp";

    public GameUrlService() {
        super(IconicGameUrlVo.class);
    ...
    public MultiValueMap<String, String> formDataBuilder(String gameCode, GameSession gameSession, Map<String, String> credentials) throws InvalidVendorLineException, InvalidFormatException {
        ...
        formData.add(apiToken,
                credentials.get(Credentials.API_TOKEN));
        formData.add("user",
        ...
        formData.add("mode",
                vendorGameCode);
        formData.add("home_url",
                gameSession.getLobbyUrl());
        formData.add("lang",
                gameSession.getVendorLanguageCode());
        formData.add("client_type",
                Platforms.checkPlatformCode(gameSession.getVendorPlatformCode()));
        formData.add("ip",
                gameSession.getIpAddress());
        formData.add("country",
                credentials.get(Credentials.COUNTRY));
        formData.add("city",
                credentials.get(Credentials.CITY));
        formData.add("homeUrl",
                gameSession.getLobbyUrl());
    ...
    @Override
    public IconicGameUrlVo callToVendor(MultiValueMap<String, String> formData, Map<String, String> credentials, GameSession gameSession, HttpRequestLog httpRequestLog) throws InvalidVendorLineException, InvalidVendorResponseException, TimeoutException {
        ...
        try {
            ResponseEntity<String> apiResponse = this.checkAndCreateAccount(formData,
                    credentials,
                    gameSession);
            if (apiResponse != null) {
        ...
        HttpHeaders httpHeaders = this.getHeaders(new HttpHeaders(),
                formData,
                credentials,
                gameSession);
        ...
        ResponseEntity<String> apiResponse2 = this.doPost(launchUrl,
                uri.toString(),
                httpHeaders,
                formData,
                isTimeout);

        this.validateResponse(apiResponse2,
                isTimeout,
                httpRequestLog,
                IconicGameUrlVo.class,
                gameSession);

        IconicGameUrlVo responseVo = new Gson().fromJson(apiResponse2.getBody(),
                IconicGameUrlVo.class);
    ...
    private ResponseEntity<String> checkAndCreateAccount(MultiValueMap<String, String> formData, Map<String, String> credentials, GameSession gameSession)
        ...
        createPlayer.put(apiToken,
                formData.get(apiToken));
        createPlayer.put("user",
                formData.get("user"));
        createPlayer.put("password",
                formData.get("user"));
        createPlayer.put("username",
                formData.get("user"));
        createPlayer.add("currency",
                gameSession.getVendorCurrencyCode());
        createPlayer.put(platform,
                formData.get(platform));
        createPlayer.put(timestamp,
