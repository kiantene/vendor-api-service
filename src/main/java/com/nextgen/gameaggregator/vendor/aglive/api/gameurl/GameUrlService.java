...
@Getter
@Setter
public class GameUrlService extends BaseGameUrlService<AgLiveGameUrlVo> {
    ...
    @Override
    public AgLiveGameUrlVo callToVendor(MultiValueMap<String, String> formData, Map<String, String> credentials, GameSession gameSession, HttpRequestLog httpRequestLog)
        ...
        this.validateResponse(response, isTimeout, httpRequestLog, AgLiveGameUrlVo.class, gameSession);

        AgLiveGameUrlVo responseVo = new AgLiveGameUrlVo();
    ...
    private void checkAndCreateAccount(GameSession gameSession, HttpRequestLog httpRequestLog)
        ...
        httpRequestLog.setUrl(this.getCheckAccountUrl() + EndPoints.CHECK_AND_CREATE_ACCOUNT);
        AtomicBoolean isTimeout = new AtomicBoolean(false);

        ResponseEntity<String> response = this.doGet(this.getCheckAccountUrl(), EndPoints.CHECK_AND_CREATE_ACCOUNT, param, isTimeout);
        this.validateResponse(response, isTimeout, httpRequestLog, AgLiveGameUrlVo.class, gameSession);

        AgLiveGameUrlVo responseVo = xmlMapper.readValue(response.getBody(), AgLiveGameUrlVo.class);
    ...
    private void createSessionToken(String credit, GameSession gameSession, HttpRequestLog httpRequestLog)
            throws InvalidVendorResponseException, TimeoutException, JsonProcessingException, InvalidVendorLineException {
