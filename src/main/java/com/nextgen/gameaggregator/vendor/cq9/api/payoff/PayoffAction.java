...
public class PayoffAction {
    ...
    public ResponseVo<CommonVo> payoff(HttpServletRequest request) {
        ...
        try {
            ...
            PayoffDto payoffDto = HttpService.convertQueryStringToDtoUrlDecode(body, PayoffDto.class);

            // 1. Validate request parameters from vendor
            this.doValidation(payoffDto, wToken);

            // 2. Gather require data
            VendorPlayer vendorPlayer = vendorPlayerService.getVendorPlayerByUsername(payoffDto.getAccount());
    ...
    private void doValidation(PayoffDto payoffDto, String wToken) throws InvalidRequestException {
        Optional.ofNullable(wToken).orElseThrow(InvalidRequestException::new);

        // General validation
        ValidationUtils.validateRequest(payoffDto);
    }
