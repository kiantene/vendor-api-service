...
public class TipsService {
    ...
    public TipsVo tips(HttpRequestLog httpRequestLog, HttpServletRequest request) {
        ...
        try {
            ...
            TipsTransactionsDto txn = dto.getMessage().getTxns().get(0);
            gameSession = gameSessionService.getGameSessionByVendorPlayerUsername(txn.getUserId());
            String gameCodeWithTable = txn.getGameCode() + "_" + txn.getTipInfo().getTableId();
            gameSession = vendorService.verifyAndRegenerateNewVendorGameCodeForGameSession(gameCodeWithTable, gameSession);
    ...
    private BalanceVo processData(TipsTransactionsDto tipsTransactionsDto, HttpServletRequest request, GameSession gameSession) {
        ...
        try {
            ...
            gameSession = vendorService.verifyAndRegenerateNewVendorGameCodeForGameSession(tipsTransactionsDto.getGameCode()+"_"+tipsTransactionsDto.getTipInfo().getTableId(), gameSession);
