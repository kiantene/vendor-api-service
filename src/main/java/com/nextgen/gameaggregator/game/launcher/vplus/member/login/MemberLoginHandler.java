...
import com.nextgen.gameaggregator.game.launcher.vplus.TokenHolder;
...
@Component
public class MemberLoginHandler extends AbstractGameLaunchHandler<MemberLoginRequest, MemberLoginResponse> {
    public MemberLoginHandler(VendorCredentialUtils credentialUtils) {
        super(credentialUtils, EndPoints.CLASS_NAME + "MemberLoginHandler", MemberLoginResponse.class, SigningStrategyType.MD5);
    }

    @Override
    public void onSuccess(GameLaunchContext context, MemberLoginResponse response) {
        TokenHolder.setToken(response.getData().getToken());
    }
    ...
    @Override
    public MemberLoginRequest buildRequestBody(GameLaunchContext context) {
    ...
    @Override
    public boolean isSuccess(MemberLoginResponse response) {
