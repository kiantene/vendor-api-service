...
public interface GameSessionRepository extends JpaRepository<GameSession, Long> {
    ...
    @Query(value = "SELECT gs FROM game_session gs WHERE gs.vendor_player_username = :vendorPlayerUsername", nativeQuery = true)
    GameSession findTop1ByVendorPlayerUsernameOrderByIdDesc(@Param("vendorPlayerUsername") String vendorPlayerUsername);

    @Query(value = "SELECT gs FROM game_session gs WHERE gs.vendor_player_username = :vendorPlayerUsername AND gs.vendor_game_code = :vendorGameCode", nativeQuery = true)
