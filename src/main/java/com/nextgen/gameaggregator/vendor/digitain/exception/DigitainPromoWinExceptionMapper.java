...
@Component(EndPoints.CLASS_NAME+"PromoWin")
public class DigitainPromoWinExceptionMapper implements VendorExceptionMapper {
    @Override
    public String getVendorClassName() {
        return EndPoints.CLASS_NAME+"PromoWin";
    }
    ...
    public VendorErrorResponse onInternalError(InternalServerException ex) {
        ...
        {
            ErrorResponse errorResponse = new ErrorResponse(ResponseCode.TRANSACTION_NOT_FOUND);
            errorResponse.setTxid(ex.getCauseAs(RollbackNotAllowedException.class).getTransactionId());
            return new VendorErrorResponse(HttpStatus.OK, errorResponse);
        }
        else if (ex.isCausedBy(EntityNotFoundException.class))
        {
            ErrorResponse errorResponse = new ErrorResponse(ResponseCode.WRONG_PLAYER_ID);
            return new VendorErrorResponse(HttpStatus.OK, errorResponse);
        }
