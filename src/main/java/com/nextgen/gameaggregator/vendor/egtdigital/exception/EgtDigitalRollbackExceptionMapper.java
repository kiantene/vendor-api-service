...
public class EgtDigitalRollbackExceptionMapper implements VendorExceptionMapper {
    @Override
    ...
    public VendorErrorResponse onGameSessionExpired(GameSessionExpiredException ex) {
        return new VendorErrorResponse(HttpStatus.OK, errorResponse(ResponseCodes.ERR_TRANSFER_ROLLED_BACK));
    }
    ...
    public VendorErrorResponse onInternalError(InternalServerException ex) {
        if (ex.isCausedBy(BetNotFoundException.class) || ex.isCausedBy(RoundNotFoundException.class)) {
            return new VendorErrorResponse(HttpStatus.OK, errorResponse(ResponseCodes.ERR_TRANSFER_DOES_NOT_EXIST));
        }
        return new VendorErrorResponse(HttpStatus.OK, errorResponse(ResponseCodes.ERR_UNKNOWN));
    }
