...
//            }
    ...
    public TransactionsListData findByAgentIdAndCreateTimeBetween(Integer agentId, TransactionsListDto dto) {
        ...
        String sqlStmt = " SELECT " +
                " bh.id AS betId, bh.round_id AS roundId, bh.external_transaction_id AS externalTransactionId, ap.username AS username, c.code AS currencyCode, vg.code AS gameCode, v.code AS vendorCode, gc.code AS gameCategoryCode, bh.bet_amount AS betAmount, bh.win_amount AS winAmount, bh.win_loss AS winLoss, bh.effective_turnover AS effectiveTurnover, bh.jackpot_amount AS jackpotAmount, bh.status AS status, bh.vendor_bet_time AS vendorBetTime, bh.vendor_settle_time AS vendorSettleTime, IF(bh.is_freespin =0 ,'FALSE','TRUE') AS isFreeSpin, bh.vendor_bet_id AS vendorBetId " +
                " FROM " +
                " (SELECT id, round_id, external_transaction_id, agent_player_id, vendor_player_id, currency_id, vendor_game_id, vendor_id, game_category_id, bet_amount, win_amount, win_loss, effective_turnover, jackpot_amount, status, vendor_bet_time, vendor_settle_time, is_freespin, vendor_bet_id " +
